...
def main(screenshot_bytes, tags):
            ...
            for video_item in video_items[1::]:
                try:
                    ...
                    if average_hash(original_image) - average_hash(img) <= 15:
                        self.titles.append(img_object["alt"])
    ...
    original_image = Image.open(screenshot_bytes).convert("RGB") # Read original image
    pages = []
    titles = []
    links = []
